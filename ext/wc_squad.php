...
<?php
	...
	for($i=0;$i<sizeof($list);$i++){
		...
		$w_manager=substr($data->item(7+2*$index)->nodeValue, 7);
		
		#$data = $finder->query("/html/body/div/div/div/table/tbody/tr/td/table");
		$data = $finder->query("/html/body/div/div/div/table/tr/td/table");
		$table = $data->item($index);
#                echoln_file($file, "# ".$data->length);
		//$players is <tbody> consist of <tr> for each player
		#$players = $finder->query("tbody/tr",$table);
		$players = $finder->query("tr",$table);
		$player_count = $players->length;
#                echoln_file($file, "# ".$player_count);
		$w_players = array();
		for($k=1;$k<$player_count;$k++){
